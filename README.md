...
| `Inklinometers.html` | Production single-file web app for offline phone use. Generated by the build script. |
| `index.html` | Development HTML source that links the separated CSS and JavaScript files. |
| `css/style.css` | Development stylesheet source. |
| `js/app.js` | Development JavaScript source. |
| `scripts/build_singlefile.py` | Build script that bundles `index.html`, `css/style.css`, and `js/app.js` into `Inklinometers.html`. |
...
Edit the separated source files during development:

- `index.html` for markup.
- `css/style.css` for styling.
- `js/app.js` for app behavior.

After source changes, rebuild the offline production file:

```powershell
python scripts/build_singlefile.py
```

Validate the current production file without rebuilding:

```powershell
python scripts/build_singlefile.py --check-only
```

The generated `Inklinometers.html` remains the file to open directly on a phone.
...
The tests use Python unittest and a Node syntax check for `js/app.js`. They cover the sensor number generator, split-source build sync, key HTML app audit regressions, box sorting feature checks, documentation sync, and the English-only text-file policy.
